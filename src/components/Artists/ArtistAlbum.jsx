...
const ArtistAlbum = ({props}) => {
    ...
    const {datas,pending,Error} = useFetch(`${URL}/artists/${idArtist}/albums`)
    console.log('Artist album ',datas) 
    return (
        <>
                ...
                {!pending && datas.items.map(album => {
                    ...
                    return (
                        ...
                        <CardBody>
                            ...
                            <Show above="md">
                                <Link href="" fontSize={{lg : '1vw',md : '1.5vw',base : '4vw'}} fontWeight={'bold'} display={'block'}>{CutString(20,album.name)}</Link>
                            </Show>
                            <Hide above='md'>
                                
                                <Link href="" fontSize={{lg : '1vw',md : '1.5vw',base : '4vw'}} fontWeight={'bold'} display={'block'}>{CutString(11,album.name)}</Link>
                            </Hide>
